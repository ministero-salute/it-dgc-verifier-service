...
public class DeltaRepository {
    ...
    public DeltaDTO getByVersions(final Long versionFrom,final Long versionTo, Long chunk) {
    ...
    public DeltaDTO getByVersionsWithoutContent(final Long versionFrom,final Long versionTo, Long chunk) {
        DeltaDTO output = null;
        try {
            Query query = new Query();
            query.addCriteria(Criteria.where("from_version").is(versionFrom).and("to_version").is(versionTo));
